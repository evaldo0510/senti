...
export default function DashboardPaciente() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Get recent mood
        userService.getMoodHistory((history) => {
          if (history.length > 0) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-24">
      {/* Header */}
      <header className="p-6 flex justify-between items-center sticky top-0 bg-slate-950/80 backdrop-blur-md z-20">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-medium text-slate-200">
              {getGreeting()}, <span className="text-emerald-400">{userProfile?.nome?.split(' ')[0] || "Paciente"}</span>
            </h1>
            <p className="text-xs text-slate-500">Pronto Socorro Emocional</p>
          </div>
        </div>
        <div className="flex gap-2">
          <button className="p-2 bg-slate-900 rounded-full border border-white/5 relative">
      ...
      <main className="p-6 max-w-2xl mx-auto space-y-8">
          ...
          <div className="relative z-10 space-y-4">
            <div className="flex justify-between items-start">
              <div className="space-y-1">
                <h3 className="text-slate-400 text-sm font-medium uppercase tracking-wider">Como você está hoje?</h3>
              ...
              <button 
                ...
                className="p-2 bg-emerald-500/10 text-emerald-400 rounded-xl border border-emerald-500/20 hover:bg-emerald-500/20 transition-colors"
              >
            ...
            <div className="flex gap-2">
              ...
              <button 
                ...
                className="flex-1 py-3 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-2xl text-sm font-bold transition-all border border-white/5 flex items-center justify-center gap-2"
              >
        ...
        {nextAppointment && (
          <motion.section 
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="bg-emerald-900/20 border border-emerald-500/20 rounded-3xl p-6 flex items-center justify-between"
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs text-emerald-400/60 uppercase tracking-widest font-bold">Próxima Sessão</p>
                ...
                <p className="text-sm text-slate-400">Com Dr(a). {nextAppointment.therapistNome}</p>
              </div>
            </div>
            <button 
              onClick={() => navigate(`/atendimento/${nextAppointment.id}`)}
              className="w-10 h-10 bg-emerald-500 text-white rounded-full flex items-center justify-center shadow-lg shadow-emerald-900/40 hover:scale-110 transition-transform"
            >
        ...
        {/* Marketplace / Uber Section */}
        <section className="space-y-4">
          <div className="flex justify-between items-end px-2">
            <div>
              <h3 className="text-xl font-medium text-slate-200">Uber da Terapia</h3>
          ...
          <div className="grid grid-cols-1 gap-4">
            {featuredTherapists.map((prof) => (
              <motion.div 
                ...
                className="bg-slate-900 border border-white/5 rounded-2xl p-4 flex items-center gap-4 cursor-pointer hover:border-emerald-500/30 transition-all"
              >
                ...
                <div className="flex-1">
                  <h4 className="font-medium text-slate-200">{prof.nome}</h4>
                  <p className="text-xs text-emerald-400/80">{prof.especialidades?.join(", ") || "Psicólogo"}</p>
                  <div className="flex items-center gap-2 mt-1">
                    <div className="flex items-center gap-0.5">
                      <Star className="w-3 h-3 text-yellow-500 fill-yellow-500" />
                      <span className="text-[10px] font-bold text-slate-400">{prof.rating?.toFixed(1) || "5.0"}</span>
                    </div>
                    <span className="text-[10px] text-slate-600">•</span>
                    <span className="text-[10px] font-bold text-slate-400">R$ {prof.preco || "150"}</span>
                  </div>
                </div>
        ...
        <section className="grid grid-cols-2 gap-4">
          <button 
            ...
            className="bg-slate-900 border border-white/5 p-6 rounded-3xl flex flex-col items-center gap-3 hover:border-emerald-500/30 transition-all"
          >
            <div className="w-12 h-12 bg-emerald-500/10 rounded-2xl flex items-center justify-center">
              <ShieldCheck className="w-6 h-6 text-emerald-400" />
            </div>
            <span className="text-sm font-medium text-slate-300">Nova Triagem</span>
          </button>
          <button 
            onClick={() => navigate("/diario")}
            className="bg-slate-900 border border-white/5 p-6 rounded-3xl flex flex-col items-center gap-3 hover:border-emerald-500/30 transition-all"
          >
            ...
            <span className="text-sm font-medium text-slate-300">Meu Diário</span>
          </button>
        </section>
        ...
        <button 
          ...
          className="w-full py-4 bg-red-500/10 border border-red-500/20 text-red-400 rounded-2xl text-sm font-bold hover:bg-red-500/20 transition-all flex items-center justify-center gap-2"
        >
      ...
      {/* Bottom Navigation */}
      <nav className="fixed bottom-0 left-0 right-0 bg-slate-950/80 backdrop-blur-lg border-t border-white/5 p-4 flex justify-around items-center z-30">
        <button 
        ...
        <button 
          onClick={() => navigate("/live-iara")}
          className="w-14 h-14 bg-emerald-500 rounded-full flex items-center justify-center -mt-10 shadow-xl shadow-emerald-900/40 border-4 border-slate-950 active:scale-90 transition-transform"
        >
